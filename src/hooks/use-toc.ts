...
export const useToc = ({
	...
	useEffect(() => {
		...
		let headingElements = Array.from(content.querySelectorAll(targetSelectors)).filter((heading) =>
		...
		if (ignoreIds) {
			headingElements = headingElements.filter((heading) => !ignoreIds.includes(heading.id))
			...
			(entries) => {
				entries.forEach((entry) => {
					if (entry.isIntersecting) {
						const index = headingElements.findIndex((heading) => heading.id === entry.target.id)
						if (index !== -1) {
							setActiveId(headingElements[index].id)
						} else {
							const closestHeading = findClosestHeading(entry.target, headingElements)
	...
	useEffect(() => {
		if (tocContainer && activeId) {
			...
			tocContainerElements.forEach((tocContainerElement) => {
				const linkElement = tocContainerElement?.querySelector(`a[href="#${activeId}"]`)
				...
				if (linkElement && tocContainerElement) {
					...
					const relativeTop = targetRect.top - parentRect.top + tocContainerElement.scrollTop

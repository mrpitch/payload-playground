...
const content = {
	previewText: 'Verify Payload Playground account',
	heading: 'Verify your account',
	salutation: 'Welcome to Payload Playground',
	copy: 'Please verify your account by clicking the button below:',
...
export function EmailVerifyAccountContent(props: EmailPasswordResetProps) {
...
export function EmailVerifyAccountPreview(props: EmailPasswordResetProps) {
	const { email } = props
		...
		<>
			<Tailwind>
				<div className="bg-background mx-auto my-auto px-2 font-sans">
					...
					<EmailVerifyAccountContent {...props} />
				</div>
			</Tailwind>
		</>
	)
...
export function EmailVerifyAccount(props: EmailPasswordResetProps) {
	const { email } = props
	const previewText = `${content.previewText} ${email}`
	return (
		<Html>
